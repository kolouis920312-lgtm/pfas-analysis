...
matplotlib 後端統一為 Agg（不開視窗、可在背景執行緒安全產圖）。
為降低網站啟動時的記憶體基線，matplotlib 不在模組載入時 import，改成
「第一次真的要畫圖／取色才載入並設定 Agg 後端」（見 _ensure_agg）。
這樣啟動（與 --max-requests 回收後的閒置 worker）都不必先吃下 matplotlib。
"""
...
def _ensure_agg():
    """第一次用到 matplotlib 前確保後端為 Agg；idempotent，實際只跑一次。"""
    global _AGG_READY
    if _AGG_READY:
        return
    import matplotlib
